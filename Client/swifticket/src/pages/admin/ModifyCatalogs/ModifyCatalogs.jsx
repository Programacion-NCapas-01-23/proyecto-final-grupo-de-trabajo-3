...
export default function ModifyCatalogs() {
    ...
    return (
        <main className="w-full flex">
            <MdKeyboardArrowLeft onClick={toggleSponsor} className="text-9xl self-center cursor-pointer"/>
            {addSponsor ? (<Sponsor/>) :( <OrganizerAndPlace />)}
            <MdKeyboardArrowRight onClick={toggleSponsor} className="text-9xl self-center cursor-pointer"/>
        </main>
    )
}
